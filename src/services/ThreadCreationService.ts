...
export default class ThreadCreationService {
	...
	public async shouldHaveThread(message: Message): Promise<boolean> {
		console.log("Checking if message should have thread:", {
	...
	public async createOrUpdateThreadOnMessage(
		...
		const requiredPermissions = getRequiredPermissions(
			channelConfig.slowmode,
			rawReplyMessageContent,
		);
		...
		const thread = await message.startThread({
			name,
			rateLimitPerUser:
				channelConfig.slowmode === 0 ? undefined : channelConfig.slowmode,
			autoArchiveDuration:
				message.channel.defaultAutoArchiveDuration ??
		...
		const replyMessageContent = await messageVariables.replace(
			rawReplyMessageContent,
		);
		if (replyMessageContent.trim().length > 0) {
			const buttonRow = await this.getButtonRow(
				channelConfig,
				messageVariables,
			);
	...
	private async getButtonRow(
		...
		const closeButtonText = clampWithElipse(
			await messageVariables.replace(config.closeButtonText),
			80,
		);
		const titleButtonText = clampWithElipse(
			await messageVariables.replace(config.titleButtonText),
			80,
		);
		...
		if (closeButtonText.length > 0) {
			const closeButton = ButtonBuilder.from(
				this.bot.getButton("close").getBuilder(),
			)
		...
		if (titleButtonText.length > 0) {
			const titleButton = ButtonBuilder.from(
				this.bot.getButton("title").getBuilder(),
			)
